		...
		struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable && !display_power_on) {
		...
		disp_en_delay = spec_pdata->disp_en_pre;
		disp_en_delay = disp_en_delay ? disp_en_delay : 5;
		usleep_range(disp_en_delay * 1000, disp_en_delay * 1000 + 100);

		gpio_set_value(vsp_gpio, 1);
		usleep_range(30 * 1000, 30 * 1000 + 100);
		gpio_set_value(vsn_gpio, 1);

		disp_en_delay = spec_pdata->disp_en_post;
		disp_en_delay = disp_en_delay ? disp_en_delay : 5;
		usleep_range(disp_en_delay * 1000, disp_en_delay * 1000 + 100);
			...
			int driver_ic, char *id_data)
{
	...
	for_each_child_of_node(parent, next) {
		...
		spec_pdata->disp_en_pre = !rc ? tmp : 0;
		rc = of_property_read_u32(next, "somc,disp-en-on-post", &tmp);
		spec_pdata->disp_en_post = !rc ? tmp : 0;
